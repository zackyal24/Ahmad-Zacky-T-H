...
// Ambil ISBN dari parameter URL
$isbn = isset($_GET['isbn']) ? $_GET['isbn'] : '';

if (empty($isbn)) {
    // Jika ISBN tidak disediakan, arahkan kembali ke halaman daftar buku atau tampilkan pesan kesalahan
    echo "ISBN tidak disediakan!";
    exit();
}

// Query untuk mengambil detail buku berdasarkan ISBN
$query_buku = "
    SELECT 
        buku.*, 
...
if ($result_buku) {
    ...
    // Ambil informasi buku dari baris hasil query
    $judul = $row_buku['judul'];
    $penerbit = $row_buku['penerbit'];
    $pengarang = $row_buku['pengarang'];
    $stok = $row_buku['stok'];
    $tgl_masuk = $row_buku['tgl_masuk'];
    $nama_kategori = $row_buku['nama_kategori'];
    $nama_rak = $row_buku['nama_rak'];
    $sampul = $row_buku['sampul'];
} else {
    // Jika query gagal, tampilkan pesan kesalahan
                    ...
                    <div class="container">
                        <div class="card profile-card">
                            <div class="card-body text-center">
                                <img src="<?= $sampul ?>" alt="Sampul <?=$judul?>" class="profile-img img-fluid">
                                <h4 class="card-title text-primary"><?= $judul ?></h4>
                                <p class="card-text">ISBN: <?= $isbn ?></p>
                                <p class="card-text" id="two">Pengarang: <?= $pengarang ?></p>
                                <p class="card-text" id="two">Penerbit: <?= $penerbit ?></p>
                                <p class="card-text" id="two">Kategori: <?= $nama_kategori ?></p>
                                <p class="card-text" id="two">Rak: <?= $nama_rak ?></p>
                                <p class="card-text" id="two">Stok: <?= $stok ?></p>
                                <p class="card-text" id="two">Tanggal Masuk: <?= $tgl_masuk ?></p>
                            </div>
                        </div>
                    </div>
                </div>
